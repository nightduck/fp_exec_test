...
int main(int argc, char * argv[])
{
  ...
  auto tmr1 = tmr_node->create_wall_timer(
    1200ms, [&](void) {
      burn_cycles_and_publish(publishers, 0, 8, 100ms, 0xDEADBEEF, -1);
    });
  auto tmr2 = tmr_node->create_wall_timer(
    2400ms, [&](void) {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    publishers.push_back(
      tmr_node->create_publisher<std_msgs::msg::Int32>(
        topic_name, qos));
    subscribers.push_back(
      sub1_node->create_subscription<std_msgs::msg::Int32>(
  ...
  for (int i = 8; i < 16; i++) {
    ...
    std::string topic_name = char_buff;
    publishers.push_back(
      sub1_node->create_publisher<std_msgs::msg::Int32>(
        topic_name, qos));
    subscribers.push_back(
      sub2_node->create_subscription<std_msgs::msg::Int32>(
        topic_name, qos, [&, i](std_msgs::msg::Int32::ConstSharedPtr msg) {
  ...
  if (argc >= 2 && std::string("fp").compare(argv[1]) == 0) {
    ...
    RCLCPP_INFO_STREAM(
      logger,
      "Running new executor from now on for " << EXPERIMENT_DURATION.count() << " seconds ...");
    ...
    RCLCPP_INFO_STREAM(
      logger,
      "Running default executor from now on for " << EXPERIMENT_DURATION.count() << " seconds ...");
    std::this_thread::sleep_for(EXPERIMENT_DURATION);
  ...
  std::cout << "Time in callbacks: " << std::atomic_load(&callback_time) / 1000000000.0 << "s" <<
    std::endl;
}
